<?php
    ...
    $getExpense=mysqli_query($conn,"SELECT * FROM expense WHERE userID=$userID");
    $getSelectedCategory=mysqli_query($conn,"SELECT expense.title FROM goal INNER JOIN expense ON goal.originalID=expense.expenseID WHERE goal.userID=$userID AND goalID=$goalID");
    $errmsg='';
