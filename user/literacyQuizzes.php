<?php
    ...
    $getQuizzes=mysqli_query($conn,"SELECT * FROM quiz");
    ...
    if(isset($_POST['search'])){
        ...
        $getQuizzes=mysqli_query($conn,"SELECT * FROM quiz WHERE title='$title'");
    }

    if(isset($_POST['categorySearch'])){
        $category=$_POST['category'];
        $getQuizzes=mysqli_query($conn,"SELECT * FROM quiz WHERE category='$category'");
    }
...
<body>
    <h2 class="pageTitle">Search Quizzes</h2>
    <form method="post">
        <div class="top">
    ...
    <div class="bottom">
        ...
        <?php else:?>
            <?php while($rowQuizzes=mysqli_fetch_assoc($getQuizzes)):?>
                <div onclick="window.location.href='quizInfo.php?quizID=<?php echo $rowQuizzes['quizID'];?>'" class="video">
